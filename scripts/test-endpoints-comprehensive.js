 ...
 * Tests all endpoints with real data from sms-blossom-dev store
 */
...
async function testEndpoint(name, method, endpoint, data = null, expectedStatus = 200, validator = null) {
  ...
  try {
    ...
    if (result.status !== expectedStatus) {
      ...
      results.failed.push({ name, endpoint, status: result.status, expected: expectedStatus, error: 'Wrong status code', data: result.data });
...
async function runTests() {
  ...
  await testEndpoint('Get Account Info', 'GET', '/settings/account', null, 200, validators.successResponse);
  ...
  // Audiences Endpoints
  console.log('\n👥 Audiences Endpoints');
  await testEndpoint('List Audiences', 'GET', '/audiences', null, 200, validators.successResponse);

  // Discounts Endpoints
  console.log('\n🎟️ Discounts Endpoints');
  await testEndpoint('Get Discounts', 'GET', '/discounts', null, [200, 500], validators.successResponse); // May fail if Shopify API not available

  // Templates Endpoints
  console.log('\n📝 Templates Endpoints');
  await testEndpoint('Get Templates', 'GET', '/templates', null, 200, validators.paginatedResponse);
  await testEndpoint('Get Template Categories', 'GET', '/templates/categories', null, 200, validators.successResponse);
  ...
  console.log(`📊 Pass Rate: ${total > 0 ? ((results.passed.length / total) * 100).toFixed(1) : 0}%`);
  ...
  if (results.failed.length > 0) {
    ...
    results.failed.forEach(test => {
      ...
      if (test.data?.message) console.log(`     Message: ${test.data.message}`);
    });
  }
